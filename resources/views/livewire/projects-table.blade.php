<table class="min-w-full divide-y divide-gray-200">
    <thead class="bg-gray-50">
        <tr>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" wire:click="sortBy('pkey')">Key</th>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-48 cursor-pointer" wire:click="sortBy('name')">Name</th>
    ...
    <tbody class="bg-white divide-y divide-gray-200">
        ...
        <tr>
            <td class="px-6 py-4 whitespace-nowrap"><a class="text-blue-500" href="{{ route('projects.edit', ['id' => $project->ID]) }}">{{ $project->PKey }}</a></td>
            <td class="px-6 py-4 whitespace-nowrap">{{ $project->Name }}</td>
            <td class="px-6 py-4">{{ $project->CreatedDate }}</td>  
            <td class="px-6 py-4 whitespace-nowrap">{{ $project->createdBy->name }}</td>
            <td class="px-6 py-4">{{ $project->UpdatedDate }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ $project->updatedBy->name }}</td>
            <td class="px-6 py-4 whitespace-nowrap align-middle text-center">
                <button wire:click="delete({{ $project->ID }})" class="text-red-500 hover:text-red-700">
